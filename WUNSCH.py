...
def change_slots(delta) : 
    for p in st.session_state.pruefer:
        p["Slots"] = p["Slots"] + delta if p["Slots"] > 0 else 0
...
with st.expander("Prüfungsslots"):
    ...
    minus = col[1].button("Alle um 1 reduzieren", on_click = change_slots, args = (-1,), help="Gilt nicht für Prüfer mit 0 Prüfungsslots")
    plus = col[2].button("Alle um 1 erhöhen", on_click = change_slots, args = (+1,), help="Gilt nicht für Prüfer mit 0 Prüfungsslots")
...
with st.expander("Upload Daten von Studierenden", expanded = False if st.session_state.xls_his is not None and st.session_state.xls_ilias is not None else True):
    ...
    with col1:
        ...
        st.write("Die Datei wird aus dem Ilias-Kurs generiert. Genauer müssen die dortigen Dateien aus den beiden Prüfungsgebieten 'Analysis I und II' sowie 'Lineare Algebra I unfd II' in eine Datei zusammengeführt werden. Sie muss Spalten 'Matrikelnummer', 'Prüfungsgebiet', 'Wiederholungsprüfung?', 'Prüfer*in Priorität 1', 'Prüfer*in Priorität 2', 'Prüfer*in Priorität 3', 'Bemerkung', 'Letzte Änderung', 'Im Besitz von (Name)' enthalten. Weitere Spalten dürfen enthalten sein und werden nicht verändert.")
        ...
        if st.session_state.xls_ilias and st.session_state.xls_his:
            ...
            for item in dict_ilias:
                ...
                if i >= 0:
                    ...
                    if item["Wiederholungsprüfung?"] == 0 and dict_his[i]["Erstprüfer"] != "":
...
if st.session_state.xls_his and st.session_state.xls_ilias:
    einteilen = st.button("Einteilen", type="primary")
    if einteilen:
